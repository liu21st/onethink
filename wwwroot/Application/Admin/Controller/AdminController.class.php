...
class AdminController extends Controller{
     ...
     * @author 朱亚杰  <zhuyajie@topthink.net>
     */
    protected function forbid($model, $where = array(), $msg = array(
        'success' => '状态禁用成功！',
        'error' => '状态禁用失败！'
    )) {
        $data = array('status' => 0);
     ...
     * @author 朱亚杰  <zhuyajie@topthink.net>
     */
    protected function resume($model, $where = array(), $msg = array(
        'success' => '状态恢复成功！',
        'error' => '状态恢复失败！'
    )) {
        $data = array('status' => 1);
     ...
     */
    protected function restore($model, $where = array(), $msg = array(
        'success' => '状态还原成功！',
        'error' => '状态还原失败！'
    )) {
        $data = array('status' => 1);
     ...
     * @author 朱亚杰  <zhuyajie@topthink.net>
     */
    protected function delete($model, $where = array(), $msg = array(
        'success' => '删除成功！',
        'error' => '删除失败！'
    )) {
        $data['status'] = -1;
    ...
    final public function getMenus($controller = CONTROLLER_NAME) {
        $menus = S('ADMIN_MENU_LIST.' . $controller . is_login());
        if (empty($menus)) {
            // 获取主菜单
            $where['pid'] = 0;
            $where['hide'] = 0;
            ...
            S('ADMIN_MENU_LIST.' . $controller . is_login(), $menus);
        }
        return $menus;
    }
    ...
    final protected function returnNodes($tree = TRUE) {
        ...
        if ((int) $tree) {
            $list = M('Menu')
                ->field('id,pid,title,url,tip,hide')
                ->order('sort asc')
                ->select();
    ...
    protected function lists($model, $where = array(), $order = '', $field = TRUE) {
        ...
        $page = new \Think\Page($total, $listRows, $GET);
        if ($total > $listRows) {

...
function JTree(pParent,xmlFile){
	...
	var createCaption=function(pNode,pLevel){
       ...
       tmp.ondblclick=function(){ 
			...
			self.clickItem=clickItem;
			self.selectNode=pNode;
			try{self.ondblclick();}catch(e){}//必须加上，如果self没有对onclick赋值的话，会引发错误。
       }		
	...
	this.create=function(){
		...
		len = self.root.treeNodes.length;
		for (i=0;i<len;i++)
		{
			self.root.treeNodes[i].expand(false);
		}
